...
public class MatchController {
     ...
     */
    @GetMapping
    public ResponseEntity<Response> getMatchList(@RequestBody @Validated RequestSearchMatch searchMatch,
                                                 BindingResult bindingResult,
                                                 Pageable pageable) {
        if (bindingResult.hasErrors()) {
            throw new BindingException(bindingResult);
        }
        List<ResponseMatchData> matchList = matchService.findAllByMatchData(searchMatch, pageable);
